...
const configuration={name:'Saved postings: preview in Firefox',type:'node',request:'launch',program:`${'${workspaceFolder}'}/${relative}/preview.mjs`,cwd:`${'${workspaceFolder}'}/${relative}`,env:{POSTINGS_OPEN_FIREFOX:'1'},console:'integratedTerminal',skipFiles:['<node_internals>/**']};
...
const index=launch.configurations.findIndex(c=>c.name===configuration.name);
...
console.log('Installed Saved postings: preview in Firefox in '+target);
