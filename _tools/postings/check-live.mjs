...
const routes=['/postings/','/postings/manifest.json','/postings/assets/style.css','/postings/assets/app.js',...manifest.bursts.flatMap(b=>[b.path,`/postings/${b.year}/`]),...manifest.entries.flatMap(e=>[e.path,e.path+'saved.html',...(e.groupPath?[`/postings/${e.burst}/${e.groupPath}/`]:[])]),...manifest.attachments.map(a=>a.path)];
...
console.log(`Verified ${count} ${origin} URLs against generated files, including all ${manifest.entries.length} saved-page copies.`);
